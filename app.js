...
app.get("/user/tweets/", authenticationToken, async (request, response) => {
  ...
  * 
  ...
  let userTweets = await database.all(userTweetsQuery);
  let userLikes = await database.all(userLikeQuery);
  let userReply = await database.all(userReplyQuery);

  let replyCount = 0;
  let final = [];
  for (let each of userTweets) {
    each.likes = 0;
    each.replies = 0;
    let likesCount = 0;
    let replyCount = 0;
    for (let each_like of userLikes) {
      if (each.tweet_id === each_like.tweet_id) {
        likesCount += 1;
      }
    }
    for (let each_reply of userReply) {
      if (each.tweet_id === each_reply.tweet_id) {
        replyCount += 1;
      }
    }
    each.likes = likesCount;
    each.replies = replyCount;
    final.push(each);
  }
  response.send(
    final.map((each) => {
